...
public class MovieRecommenderServlet extends HttpServlet {
	...
	private static final long serialVersionUID = 1011280663430028955L;
	...
	private static void writeJSON(HttpServletResponse response, RecommendMovieList movieList) throws IOException {
	...
	private void writePlainText(HttpServletResponse response, long userID, boolean debug,
			Iterable<RecommendedItem> items, RecommendMovieList movieList) throws IOException, TasteException {
		response.setContentType("text/plain");
		response.setCharacterEncoding("UTF-8");
		response.setHeader("Cache-Control", "no-cache");
		PrintWriter writer = response.getWriter();
	...
	private static void writeRecommendations(RecommendMovieList movieList, PrintWriter writer) {
